...
SET PG_COMMAND=docker run -d --name !PG_CONTAINER_NAME! -e "POSTGRES_USER=postgres" -e "POSTGRES_PASSWORD=123456" -e "POSTGRES_DB=myspringdatabase" -p 5432:5432 -v !PG_VOLUME_NAME!:/var/lib/postgresql !POSTGRESQL_IMAGE!
...
SET KEYCLOAK_COMMAND=docker run --name !KEYCLOAK_CONTAINER! --detach -p 8080:8080 -p 9000:9000 -e "KC_BOOTSTRAP_ADMIN_USERNAME=!KEYCLOAK_OVERLORD!" -e "KC_BOOTSTRAP_ADMIN_PASSWORD=!KEYCLOAK_ADMIN_PASSWORD!" -e "KC_HEALTH_ENABLED=true" -e "KC_METRICS_ENABLED=true" -v keycloak-volume:/opt/keycloak/data !KEYCLOAK_IMAGE! start-dev
