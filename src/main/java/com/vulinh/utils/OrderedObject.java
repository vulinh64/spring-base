 ...
 * sorting operations.
 *
 * <p>This class ensures that the wrapped value is never null and the order is never negative.
 *
...
public record OrderedObject<T>(T value, int order) {
  ...
  public static class Wrapper {
    ...
    /**
     * Creates a Comparator that first compares OrderedObject instances by their natural order using
     * the provided key extractor function, then by their order value. This method uses ascending
     * order and places null values last.
     *
    ...
    /**
     * Creates a Comparator that first compares OrderedObject instances using the provided key
     * extractor function, then by their order value. The comparison order and null handling
     * strategy can be specified.
     *
     * @param <T> the type of objects wrapped in OrderedObject
     * @param <U> the type of the key used for comparison, must be Comparable
